...
### **Сценарий 1: Мониторинг входа/выхода**
1. Сотрудник прикладывает карту к турникету
2. БОЛИД АРМ ОРИОН ПРО регистрирует событие
3. Система отправляет email на localhost:1025
4. Приложение обрабатывает событие и отправляет уведомление в Telegram
5. Администратор безопасности получает мгновенное уведомление

### **Сценарий 2: Обработка нарушений**
1. Попытка несанкционированного доступа
2. БОЛИД генерирует событие нарушения
3. Email отправляется в приложение
4. Все авторизованные пользователи получают уведомление
5. Возможность быстрого реагирования на инцидент

### **Сценарий 3: Формирование отчетов**
1. Администратор запрашивает отчет за период
2. Система анализирует события из базы данных
3. Генерируется HTML отчет с детализацией
4. Отчет отправляется в Telegram
5. Возможность анализа трендов и паттернов доступа
...
- **События безопасности:**
  - Попытки обхода системы доступа
  - Нарушения графика доступа
  - Подозрительная активность

- **Системные события:**
  - Ошибки связи с оборудованием
